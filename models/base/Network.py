...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        self.mode = mode
        self.args = args
        if self.args.dataset in ['cifar100', 'mnist']:
        ...
        self.ex_fc = self.args.base_class * 2
        # self.fc = nn.Linear(self.num_features, self.pre_allocate, bias=False)
        self.fc = nn.Linear(self.num_features, self.ex_fc, bias=False)
    ...
    def pre_encode(self, x):
        ...
        elif self.args.dataset in ['mini_imagenet', 'cub200']:
            x = self.encoder.conv1(x)
            x = self.encoder.bn1(x)
            x = self.encoder.relu(x)
